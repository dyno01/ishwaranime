...
class Extensions {
  ...
  static Future<void> setRepo(ItemType itemType, String repo) async {
    repo.trim();
    if (repo.endsWith('index.json')) {
      snackString('Dartotsu does not support index.json repo, use index.min.json');
      return;
    }
    if (itemType == ItemType.manga) {
        ...
        () {
          ...
          if (text.endsWith('index.json')) {
            AlertDialogBuilder(context)
              ...
              ..setMessage('Dartotsu does not support index.json repo, use index.min.json')
